...
const App: React.FC = () => {
  ...
  const handleBatchTranslateAndEmbed = async (files: string[], knowledgeBaseId: string): Promise<void> => {
    try {
      ...
      // 1. 創建翻譯文件資料夾
      const createFolderResponse = await fetch('http://localhost:5000/api/files/create_folder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          path: 'translated_files'  // 創建翻譯後檔案資料夾
        })
      });
      ...
      translatedFormData.append('path', 'translated_files');  // 指定保存到翻譯後檔案資料夾
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      <Sidebar
        ...
        translatedFiles={fileProcessing.translatedFiles}  // 添加這行
      />
      ...
      <div className="flex-1 ml-64 p-6">
        {currentMode === "files" ? (
          ...
          />
        ) : currentMode === "translated-files" ? (  // 添加新的條件渲染
          <TranslatedFilesView files={fileProcessing.translatedFiles} />
        ) : null}
      </div>
    </div>
  );
