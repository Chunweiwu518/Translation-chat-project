...
const App: React.FC = () => {
  ...
  const handleBatchTranslateAndEmbed = async (
    ...
    try {
      ...
      for (const file_path of files) {
        ...
        // 上傳翻譯後的文件
        const uploadTranslatedResponse = await fetch('http://localhost:5000/api/files/upload', {
          method: 'POST',
          body: translatedFormData,
        });

        if (!uploadTranslatedResponse.ok) {
  ...
  const handleFileChat = async (files: string[]) => {
    try {
      // 創建新的對話會話
      chat.createNewChatSession('default');  // 使用預設知識庫
      ...
      const successMessage: Message = {
        sender: "system",
        text: `已成功載入 ${files.length} 個檔案到預設知識庫，您可以開始詢問相關問題。`,
      };
