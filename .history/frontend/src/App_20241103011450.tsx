    ...
    try {
      ...
      for (const filePath of allFiles) {
        try {
          ...
          // 從檔案系統讀取檔案內容
          const fileResponse = await fetch(`http://localhost:5000/api/files/content/${encodeURIComponent(filePath)}`);
          if (!fileResponse.ok) {
          ...
          // 更新翻譯文件檢視 - 使用更穩定的 ID
          const newTranslatedFile: TranslatedFile = {
          ...
          fileProcessing.setTranslatedFiles(prev => {
            ...
            if (existingIndex >= 0) {
              // 如果存在，更新該檔案
              const newFiles = [...prev];
              newFiles[existingIndex] = newTranslatedFile;
              return newFiles;
            }
            // 如果不存在，添加新檔案
            return [...prev, newTranslatedFile];
          });
          ...
          fileProcessing.setTranslatedFiles(prev => [...prev, failedFile]);
          currentFileIndex++;
          continue;
        }
      }
