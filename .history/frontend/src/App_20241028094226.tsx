...
const App: React.FC = () => {
  ...
  // 批次翻譯並加入知識庫
  const handleBatchTranslateAndEmbed = async (files: string[], knowledgeBaseId: string) => {
    for (const fileId of files) {
      try {
        ...
        if (translateResponse.ok) {
          ...
          // 3. 加入知識庫
          await fetch('http://localhost:5000/api/embed', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        console.error(`處理檔案 ${fileId} 時出錯:`, error);
      }
    }
  };

  // 批次加入知識庫
  const handleBatchEmbed = async (files: string[], knowledgeBaseId: string) => {
    for (const fileId of files) {
      try {
        // 1. 獲取檔案內容
        const response = await fetch(`http://localhost:5000/api/files/${fileId}`);
