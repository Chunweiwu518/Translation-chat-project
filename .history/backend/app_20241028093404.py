...
# 新增檔案管理相關的路由
@app.get("/api/files")
async def get_files(category: Optional[str] = None, search: Optional[str] = None):
    """獲取檔案列表"""
    try:
        ...
        for file_path in files_path.glob("*"):
            if category and not file_path.name.startswith(category):
                continue
            ...
            files.append(
                FileInfo(
                    ...
                    category=file_path.parent.name if category else None,
                )
            )
