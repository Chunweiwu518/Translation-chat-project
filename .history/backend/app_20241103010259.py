...
class EmbedRequest(BaseModel):
    content: Union[str, List[str]]  # 修改這裡，允許字串或字串列表
    filename: str
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"直接讀取文本失敗: {str(e)}")
                raise ValueError(f"無法讀取文件: {file_path}")
...
async def upload_and_translate(file: UploadFile = File(...)):
    ...
    try:
        print(f"開始處理文件: {file.filename}")  # 添加日誌
        # 讀取上傳的檔案內容
        content = await file.read()
        content_text = content.decode('utf-8')  # 解碼文件內容

        print("開始翻譯文件內容")  # 添加日誌
        # 執行翻譯
        translated_content = one_chunk_translate_text(
            content_text,
            Config.MODEL_NAME,
            Config.SOURCE_LANG,
            Config.TARGET_LANG,
        ...
        print(f"翻譯結果已保存: {file_id}")  # 添加日誌
        return translation_data

    except Exception as e:
        print(f"翻譯過程中出錯: {str(e)}")  # 添加日誌
        raise HTTPException(status_code=500, detail=f"翻譯過程中出錯: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            if isinstance(request.content, list):
                ...
                # 使用 add_texts 方法添加內容
                temp_store.add_texts(
                    texts=[request.content], metadatas=[metadata], ids=[doc_id]
                )
                temp_store.persist()
                print(f"已添加文檔，ID: {doc_id}")
                result_doc_id = doc_id

            return {"success": True, "doc_id": result_doc_id}

        finally:
            # 如果不是當前知識庫，清理資源
            if kb_id != Config.current_kb_id:
                try:
                    if hasattr(temp_store._client, "reset"):
                        temp_store._client.reset()
                except Exception as e:
                    print(f"清理資源時出錯: {str(e)}")
    ...
    try:
        ...
        try:
            ...
            if kb_id != Config.current_kb_id and temp_store:
                try:
                    if hasattr(temp_store, "_client") and hasattr(
                        temp_store._client, "reset"
                    ):
                        temp_store._client.reset()
                    # 不再調用 close 方法
                except Exception as e:
                    print(f"清理向量存儲資源時出錯: {str(e)}")
...
@app.get("/api/files/translated_content/{file_path:path}")
async def get_translated_file_content(file_path: str):
    try:
        # 確保路徑安全
...
async def save_translation(translation: dict):
    """保存翻譯結果"""
    try:
        translations_file = Path("translations/translations.json")
        translations_file.parent.mkdir(parents=True, exist_ok=True)
...
async def get_files_recursive(path: str):
    ...
    try:
        base_path = Path(Config.UPLOAD_FOLDER)
        folder_path = base_path / path.lstrip("/")
        ...
        def collect_files(current_path: Path, relative_to: Path):
            for item in current_path.iterdir():
                ...
                if item.is_file() and allowed_file(item.name):
                    ...
                    collect_files(item, relative_to)
                    
        collect_files(folder_path, base_path)
        return files
