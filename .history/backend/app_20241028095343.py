...
# 新增檔案管理相關的路由
@app.get("/api/files")
async def get_files(search: Optional[str] = None, category: Optional[str] = None):
    """獲取檔案列表"""
    try:
        files_path = Path(Config.UPLOAD_FOLDER)
        if not files_path.exists():
            return []

        files = []
        for file_path in files_path.rglob("*"):  # 使用 rglob 來遞迴搜尋所有檔案
            if file_path.is_file():
                ...
                file_category = (
                    ...
                    if file_path.parent.name != Config.UPLOAD_FOLDER
                    else None
                )
                ...
                file_info = FileInfo(
                    id=str(file_path.relative_to(files_path)),  # 使用相對路徑作為 ID
                    name=file_path.name,
...
async def upload_file(file: UploadFile = File(...), category: Optional[str] = None):
    """上傳檔案"""
    if not file or not file.filename:
        raise HTTPException(status_code=400, detail="沒有提供文件")
    ...
    try:
        save_path = Path(Config.UPLOAD_FOLDER)
        save_path.mkdir(parents=True, exist_ok=True)

        if category:
            save_path = save_path / category
            save_path.mkdir(parents=True, exist_ok=True)
