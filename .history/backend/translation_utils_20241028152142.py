...
def translate_and_store_to_knowledge_base(
    file_path, model, source_lang, target_lang, country, progress_callback=None
):
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                translated_text = one_chunk_translate_text(
                    chunk.page_content, model, source_lang, target_lang, country
                )
                # 移除多餘的空行和空格
                translated_text = translated_text.strip()

                # 如果不是第一個塊，檢查是否需要添加連接詞
                if (
                    i > 0
                    ...
                    if not last_chunk.endswith("。") and not last_chunk.endswith("，"):
                        translated_text = "，" + translated_text

                translated_chunks.append(translated_text)
                print(f"Translated chunk {i+1}/{total_chunks}")
        ...
        for i, chunk in enumerate(translated_chunks):
            if i > 0:
                ...
                if (
                    ...
                    and not full_translation.endswith("。")
                    and not full_translation.endswith("，")
                ):
                    full_translation += "，"
                elif not chunk.startswith("。") and full_translation.endswith("。"):
                    full_translation += " "  # 在句號後添加空格
            full_translation += chunk

        # 最終清理
        # 1. 移除連續的標點符號
        while "。。" in full_translation:
